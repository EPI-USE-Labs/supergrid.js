(function (factory) {
    ...
    SuperGrid.prototype.layout = function (resizing) {
        ...
        switch (this.options.layout) {
            case 'classic':
                this._layoutClassic(resizing);
                break;
            case 'classicFixedRowHeight':
                this._layoutPackedGrid(resizing);
                break;
            default:
                this._layoutClassic(resizing)
        }
    ...
    SuperGrid.prototype._layoutClassic = function (resizing) {
        var self = this;
    ...
    SuperGrid.prototype._layoutPackedGrid = function (resizing) {
        // TODO: implement

        /**
         * Implementation during Slack call
         */
        ...
        var sortedBlocks = _.sortBy(this.blocks, function (n) {
            return n.y * 6.0 + ((n.id == self.draggingNodeId) ? 0.0 : 0.5) + (n.x);
        });

        _.each(sortedBlocks, function (block) {
            if (block.id == self.draggingNodeId) {
                block.y = previousRowY;
                return;
            }

            if (currentX + block.width > 6) {
                previousRowY += currentRowHeight;
                currentRowHeight = 0;
                currentX = 0;
            }
        ...
        self.containerHeight = previousRowY + currentRowHeight;
    };
